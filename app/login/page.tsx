...
export default function LoginPage() {
  ...
  // Detecta qué proveedores están realmente configurados (con credenciales
  // cargadas). Sin esto, si algún día falta GOOGLE_CLIENT_ID el botón de
  // Google queda ahí, visible, pero rompe al tocarlo.
  const [proveedores, setProveedores] = useState<string[]>([]);
  useEffect(() => {
    getProviders().then((p) => setProveedores(p ? Object.keys(p) : []));
  }, []);
    ...
    <div className="min-h-screen bg-[#14110C] text-[#F5F1E6] font-body flex flex-col">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="kk-panel p-8 rounded-3xl max-w-md w-full">
          ...
          <div className={`grid gap-3 ${proveedores.includes('google') ? 'grid-cols-2' : 'grid-cols-1'}`}>
