...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-orange-500 selection:text-white">
      ...
      {/* NAVEGACIÓN PROFESIONAL (Efecto Cristal + Menú Móvil) */}
      <header className="flex flex-col md:flex-row md:items-center justify-between p-4 md:px-8 border-b border-white/5 bg-[#050505]/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="flex items-center justify-between w-full md:w-auto">
          ...
          <Link href="/" className="flex items-center gap-3 hover:opacity-80 transition group">
            <img 
              src="/logo.jpg" 
              alt="Kitson Kit Logo" 
              className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-transparent group-hover:border-orange-500 transition duration-300"
            />
        ...
        <nav className={`${isMobileMenuOpen ? 'flex' : 'hidden'} md:flex flex-col md:flex-row gap-6 mt-6 md:mt-0 font-medium text-sm text-gray-400 w-full md:w-auto items-center`}>
          ...
          <Link href="/soporte" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-orange-400 transition">Soporte</Link>
        </nav>
      ...
      <section className="border-y border-white/5 bg-white/[0.01] py-16">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-10 text-center">
          <div className="flex flex-col items-center">
            <div className="w-14 h-14 bg-orange-500/10 rounded-2xl flex items-center justify-center text-2xl mb-5 border border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.1)]">⚡</div>
            <h3 className="text-white font-bold text-lg mb-2">Entrega Instantánea</h3>
            <p className="text-gray-400 text-sm">Recibe tus pedidos en cuestión de segundos tras confirmar el pago.</p>
          </div>
          <div className="flex flex-col items-center">
            <div className="w-14 h-14 bg-yellow-500/10 rounded-2xl flex items-center justify-center text-2xl mb-5 border border-yellow-500/20 shadow-[0_0_15px_rgba(234,179,8,0.1)]">🔒</div>
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-gray-400 text-sm">Estamos disponibles en Discord para ayudarte en cualquier momento.</p>
          </div>
        </div>
      </section>
      ...
      {/* FOOTER PROFESIONAL */}
      <footer className="border-t border-white/5 bg-[#020202] pt-20 pb-10 mt-10">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-10 mb-16">
          <div className="md:col-span-2">
            <div className="flex items-center gap-3 mb-6">
              <img src="/logo.jpg" alt="Logo Footer" className="w-10 h-10 rounded-full opacity-80 grayscale group-hover:grayscale-0 transition" />
          ...
          <div>
            <h4 className="text-white font-bold mb-5 text-lg">Ayuda legal</h4>
            <ul className="space-y-3 text-sm text-gray-400 font-medium">
              <li><Link href="#" className="hover:text-orange-400 transition">Términos y Condiciones</Link></li>
              <li><Link href="#" className="hover:text-orange-400 transition">Política de Reembolsos</Link></li>
              <li><Link href="/soporte" className="hover:text-orange-400 transition">Contacto y Soporte</Link></li>
            </ul>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-4">
            <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-orange-500 hover:text-white transition">X</a>
            <a href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-gray-400 hover:bg-[#5865F2] hover:text-white transition">D</a>
          </div>
        </div>
      </footer>
