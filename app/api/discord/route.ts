...
// AGREGAMOS LA NUEVA FUNCIÓN ejecutarEntregaManual AQUÍ:
import { ejecutarEntregaConCuenta, ejecutarEntregaManual } from '../../../lib/entregas';
...
export async function POST(req: Request) {
  try {
    ...
    if (interaction.type === 3) {
      ...
      // ==============================================================================
      // AQUÍ ESTÁ LA MAGIA: El botón normal usa ejecutarEntregaManual() ahora
      // ==============================================================================
      if (customId.startsWith('entregar_')) {
        const orderId = customId.replace('entregar_', '');

        // Respondemos a Discord inmediatamente para no agotar los 3 segundos
        // y ejecutamos la entrega, el envío a Sheets y el borrado en segundo plano.
        ejecutarEntregaManual(orderId)
          .then(async (resultado) => {
            const emojiTitulo = resultado.ok ? `✅ **ORDEN #${orderId.slice(0,8)} ENTREGADA EXITOSAMENTE**` : `❌ **ERROR AL ENTREGAR**`;
            ...
            await fetch(
              `https://discord.com/api/v10/webhooks/${interaction.application_id}/${interaction.token}/messages/@original`,
              {
                ...
                body: JSON.stringify({
                  content: `${emojiTitulo}\n${resultado.resumen}`,
                  // Si salió bien, borramos la tarjeta visual de Discord, si falló la dejamos.
                  embeds: resultado.ok ? [] : interaction.message?.embeds || [], 
                  components: resultado.ok ? [] : interaction.message?.components || [],
                }),
              }
            ).catch((e) => console.error('Error actualizando mensaje Discord:', e));
          })
          .catch((e) => console.error('Error en entrega manual:', e));

        // Le avisamos a Discord que la acción está en proceso
